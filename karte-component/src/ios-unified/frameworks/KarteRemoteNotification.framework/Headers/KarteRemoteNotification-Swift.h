...
#ifndef KARTEREMOTENOTIFICATION_SWIFT_H
#define KARTEREMOTENOTIFICATION_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="KarteRemoteNotification",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS_NAMED("RemoteNotification")
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class) BOOL isEnabledAutoMeasurement SWIFT_DEPRECATED_MSG("", "RemoteNotificationConfiguration.isEnabledAutoMeasurement");)
...
///   </li>
///   <li>
///     メッセージ中に <code>URL</code> が含まれていない場合
///   </li>
///   <li>
///     不正なURL
///   </li>
...
/// \param userInfo リモート通知メッセージ
///
- (nullable instancetype)initWithUserInfo:(NSDictionary * _Nonnull)userInfo OBJC_DESIGNATED_INITIALIZER;
...
/// 内部では、メッセージ中に含まれるURLを <code>UIApplication.open(_:options:completionHandler:)</code> に渡す処理を行っています。
/// <code>UIApplication.open(_:options:completionHandler:)</code> の呼び出しが行われた場合は <code>true</code> を返し、メッセージ中にURLが含まれない場合は <code>false</code> を返します。
- (BOOL)handle;
/// 通知のクリック計測を行います。<br>
/// 通常は自動でクリック計測が行われるため本メソッドを呼び出す必要はありませんが、
/// <code>isEnabledAutoMeasurement</code> が <code>false</code> の場合は自動での計測が行われないため、
/// 本メソッドを呼び出す必要があります。
- (void)track;
